...
public abstract class Database {
    ...
    public void addTransaction(ShopTransactionResult transaction){
        ...
        try {
            ...
            int itemCount = transaction.getShopItem().getItem().getAmount();
            int isBuy;
            if(transaction.getShopAction() == ShopManager.ShopAction.BUY) isBuy = 1;
            else isBuy = 0;
            System.out.println("INSERT INTO " + table + " (player, item_name, amount, is_buy, transaction_time) VALUES(" + uuid + ", " +  item + ", " + itemCount + ", " + isBuy + ", current_timestamp )");
            System.out.println("succesfully hooked transaction from " +
                    transaction.getPlayer() + " who bought " +
                    + itemCount + " of " + item + " where transaction type is " + isBuy);
            ps = conn.prepareStatement("INSERT INTO " + table + " (player, item_name, amount, is_buy, transaction_time) VALUES( \""+ uuid + "\", \"" +  item + "\", " + itemCount + ", " + isBuy + ", current_timestamp )");
//            ps.setString(1, player.getName().toLowerCase());                                             // YOU MUST put these into this line!! And depending on how many
//            // colums you put (say you made 5) All 5 need to be in the brackets
//            // Seperated with comma's (,) AND there needs to be the same amount of
//            // question marks in the VALUES brackets. Right now i only have 3 colums
